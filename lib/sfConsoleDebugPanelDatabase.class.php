...
*/
class sfConsoleDebugPanelDatabase extends sfWebDebugPanel
{
	/**
	 * Constructor.
	 *
	...
	{
		$newLogs = array();
		foreach ($logs as $log) if (!$this->isDbLog($log)) $newLogs[] = $log;
		return $newLogs;
	}
	...
	protected function getSqlLogs()
	{
		...
		foreach ($logs as $i => $log) if (count($bindings[$i])){
			$logs[$i]['query'] .= sprintf(' (%s)', implode(', ', $bindings[$i]));
		}

		$sqlLogs[] = array(
			'time' => isset($queryMatches[1]) ? $queryMatches[1] : '-',
