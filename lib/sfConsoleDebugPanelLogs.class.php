...
class sfConsoleDebugPanelLogs extends sfWebDebugPanelLogs
{
	...
	public function getPanelContent()
	{
		...
		foreach ($logs as $log){
			...
			// xdebug information
			$debug_info = array();
			if (isset($log['debug_stack']) && count($log['debug_stack'])){
				foreach ($log['debug_stack'] as $i => $logLine){
					array_push($debug_info, array(
						'line_nb' => $i,
	...
	protected function formatLogLine($logLine)
	{
		...
		$logLine = sfToolkit::pregtr($logLine, array(
			'/&quot;(.+?)&quot;/s'  => '"\\1"',
			'/^(.+?)\(\)\:/S'       => '\\1():',
			'/line (\d+)$/'         => 'line \\1'));
